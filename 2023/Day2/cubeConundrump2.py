...
"""
since data structure has colour arranged by r , g, b do a runningsum on each index.
"""
...
for i in range(1,101):
    ...
    green = 0
    blue = 0
    for subset in range(len(games[i])):
          red = max(red, games[i][subset][0])
          green = max(green, games[i][subset][1])
          blue = max(blue, games[i][subset][2])
    
    cubesum+= red * green* blue 
print(cubesum)
